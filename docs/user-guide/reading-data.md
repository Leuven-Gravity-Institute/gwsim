...
We use the [GWpy](https://gwpy.github.io/) Python package for these examples. For more details, refer to the [GWpy documentation](https://gwpy.github.io/docs/).
...
data = TimeSeries.read("filename.gwf", channel="H1:STRAIN")
```

**Parameters:**
- `filename`: Path to the GWF file
- `channel`: Channel name to read (format: `DETECTOR:CHANNEL_NAME`)
...
# Read H1 strain data
h1_data = TimeSeries.read("H1-NOISE-1000000000-4.gwf", channel="H1:STRAIN")

# Check properties
print(f"Duration: {h1_data.duration}")
print(f"Sampling frequency: {h1_data.sample_rate}")
print(f"Start time: {h1_data.t0}")
```

## Merging Frame Files

Frame files generated by gwsim may contain different types of content (noise, signals, glitches). To obtain a realistic data stream, merge multiple files:

```python
from gwpy.timeseries import TimeSeries

# Read noise and signal data
noise_data = TimeSeries.read("filename_noise.gwf", channel="H1:STRAIN")
signal_data = TimeSeries.read("filename_signal.gwf", channel="H1:STRAIN")
...
files = [
    ...
    "H1-NOISE-1000000004-4.gwf",
    "H1-NOISE-1000000008-4.gwf"
]

# Read all files
data_list = [TimeSeries.read(f, channel="H1:STRAIN") for f in files]
...
# Read metadata
with open("metadata/basic_noise-0.metadata.yaml", "r") as f:
    import yaml
...
for detector in detectors:
    ...
    filename = f"{detector}-NOISE-1000000000-4.gwf"
...
# Read data
data = TimeSeries.read("H1-NOISE-1000000000-4.gwf", channel="H1:STRAIN")
