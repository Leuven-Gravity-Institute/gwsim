"""Unit tests for the adapter-backed SignalSimulator base class."""
...
def signal_simulator_with_mocks(tmp_path):
    """Create a SignalSimulator with mixin initialization patched out."""
    ...
    with (
        ...
        patch("gwmock.signal.base.SignalAdapter.from_source_type", return_value=mock_adapter) as mock_from_source_type,
    ):
...
class TestSignalSimulatorInitialization:
    """Test SignalSimulator initialization."""

    def test_init_resolves_source_type_adapter(self, signal_simulator_with_mocks):
        """The simulator should resolve its backend through the public source-type registry."""
        simulator, _, mock_from_source_type = signal_simulator_with_mocks
        assert simulator.detectors == ["H1"]
...
class TestSignalSimulatorSimulate:
    ...
    def test_simulate_returns_timeseries_list(self, signal_simulator_with_mocks):
        ...
        simulator, _, _ = signal_simulator_with_mocks
    ...
    def test_simulate_calls_signal_adapter_with_segment_context(self, signal_simulator_with_mocks):
        """`_simulate` should forward event parameters and segment settings to the adapter."""
        expected_sampling_frequency = 4096.0
        expected_minimum_frequency = 5.0
        simulator, mock_adapter, _ = signal_simulator_with_mocks
        parameters = {
        ...
        assert call.kwargs["sampling_frequency"] == expected_sampling_frequency
        assert call.kwargs["minimum_frequency"] == expected_minimum_frequency
    ...
    def test_simulate_stops_after_first_chunk_starting_outside_segment(self, signal_simulator_with_mocks):
...
class TestSignalSimulatorMetadata:
    ...
    def test_metadata_includes_signal_configuration(self, signal_simulator_with_mocks):
...
class TestSignalSimulatorUpdateState:
    ...
    def test_update_state_advances_counter_and_start_time(self, signal_simulator_with_mocks):
